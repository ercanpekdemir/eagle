...
public class ProfileFirmaController implements Initializable {
    ...
    public Label firma_name;
    public Label firma_tel;
    public TextArea firma_adres;
    public Label firma_totalPurchase;
    public Label firma_totalCashPayment;
    public Label firma_totalCCPayment;
    public Label firma_totalCEKPayment;
    public Label firma_totalReturnPayment;
    public Label firma_balance;
    public Label firma_total_tahsilat;

    public AnchorPane profile_firma;
    ...
    public ProfileFirmaController() {}

    public ProfileFirmaController(Initializable sceneController, User user) {
...
//        dummyData();
        ...
        firma_name.setText(user.name);
        firma_tel.setText(user.tel);
        firma_adres.setText(user.address);
        firma_totalPurchase.setText(CalculatorUtil.amountFormatter(balance.totalPurchase));
        firma_totalCashPayment.setText(CalculatorUtil.amountFormatter(balance.totalCashPayment));
        firma_totalCCPayment.setText(CalculatorUtil.amountFormatter(balance.totalCCPayment));
        firma_totalCEKPayment.setText(CalculatorUtil.amountFormatter(balance.totalCEKPayment));
        firma_totalReturnPayment.setText(CalculatorUtil.amountFormatter(balance.totalReturnPayment));
        firma_total_tahsilat.setText(CalculatorUtil.amountFormatter(balance.totalTahsilat));
        firma_balance.setText(CalculatorUtil.amountFormatter(balance.balance));
                ...
                return new TableCell<>() {
                    private final MyButton btn = new MyButton("Düzenle");
                    {
                        btn.setOnAction((ActionEvent event) -> {
                            try {
                                int colIndex = ((MyButton)event.getSource()).getIndex()-1;
                                PaymentTransactionData paymentData = paymentList.get(colIndex);
                                sceneController.addNewTab("/edit_payment.fxml", Naming.FIRMA_PAYMENT_EDIT_TAB,"edit_payment", new EditPaymentController(sceneController, user, paymentData));
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                        });
                    ...
                    private final MyButton btn = new MyButton("Düzenle");
                    {
                        btn.setOnAction((ActionEvent event) -> {
                            try {
                                ...
                                sceneController.addNewTab("/edit_purchase.fxml", Naming.FIRMA_PURCHASE_EDIT_TAB,"edit_purchase", new EditPurchaseController(sceneController, user, purchaseData));
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                        });
    ...
    public void onClickFirmaDuzenle(MouseEvent mouseEvent) throws IOException {
        ...
        sceneController.addNewTab("/edit_firma.fxml", Naming.FIRMA_EDIT_TAB,"edit_firma", new EditFirmaController(sceneController, user, userData));
    }

    public void onClickFirmaYazdir(MouseEvent mouseEvent) {
        try {
            printNode(profile_firma);
    ...
    public void onClickFirmaYeniPayment(MouseEvent mouseEvent) throws IOException {
        sceneController.addNewTab("/yeni_payment.fxml", Naming.FIRMA_NEW_PAYMENT_TAB,"yeni_payment", new YeniPaymentController(sceneController, user));
    }

    public void onClickFirmaYeniPurchase(MouseEvent mouseEvent) throws IOException {
        sceneController.addNewTab("/yeni_purchase.fxml", Naming.FIRMA_NEW_PURCHASE_TAB,"yeni_purchase", new YeniPurchaseController(sceneController, user));
    }
